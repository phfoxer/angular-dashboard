...
export class FuncionariosListComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.funcionariosService.list(this.filter).then(result => {
      this.FuncionariosList = result.data;
      this.pagination = {
  ...
  public confirmDelete(data: any) {
    ...
    if (data.confirm) {
      this.funcionariosService.delete(data.id).then(r => {
        const toast = {
          icon: 'check',
          title: 'Removido',
        ...
        if (this.FuncionariosList.length === 1) {
          const p = (this.page - 1);
          this.page = (p > 0) ? p : 1;
          this.ngOnInit();
        }
