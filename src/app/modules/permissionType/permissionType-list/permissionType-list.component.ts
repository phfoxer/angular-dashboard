...
@Component({
  selector: 'app-permissionType-list',
  templateUrl: './permissionType-list.component.html',
  styleUrls: ['./permissionType-list.component.scss'],
  providers: [PermissionTypeService]
})
export class PermissionTypeListComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.permissionTypeService.list(this.filter).then(result => {
      this.PermissionTypeList = result.data;
      this.pagination = {
  ...
  public confirmDelete(data: any) {
    ...
    if (data.confirm) {
      this.permissionTypeService.delete(data.id).then(r => {
        const toast = {
          icon: 'check',
          title: 'Removido',
        ...
        if (this.PermissionTypeList.length === 1) {
          const p = (this.page - 1);
          this.page = (p > 0) ? p : 1;
          this.ngOnInit();
        }
